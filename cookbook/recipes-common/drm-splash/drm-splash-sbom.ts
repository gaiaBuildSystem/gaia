...
if (DO_SBOM === "true") {
    logger.info("Generating SBOM for DRM splash ...")
    ...
    let drm_splash_version = "unknown"
    drm_splash_version = execSync(
        "cat VERSION",
        {
            cwd: app_src_path,
            encoding: "utf-8",
    ...
    _cdx_sbom.components.add(
        new CDX.Models.Component(
            ...
            {
                version: `${drm_splash_version}`,
                cpe: `cpe:2.3:*:*:set_mode:${drm_splash_version}:*:*:*:*:*:*:*`,
                purl: PackageURL.fromString(`pkg:generic/set_mode@${drm_splash_version}`)
            }
        )
    )
    ...
    _cdx_sbom.components.add(
        new CDX.Models.Component(
            ...
            {
                version: `${drm_splash_version}`,
                cpe: `cpe:2.3:*:*:drm_display:${drm_splash_version}:*:*:*:*:*:*:*`,
                purl: PackageURL.fromString(`pkg:generic/drm_display@${drm_splash_version}`)
            }
        )
    )
    ...
    logger.success("SBOM generation for DRM splash completed successfully.")
}
