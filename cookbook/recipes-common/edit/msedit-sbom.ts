...
if (DO_SBOM === "true") {
    logger.info("Generating SBOM for MSEdit ...")

    // get the version
    // the version from msedit comes from the recipe json file

    let msedit_version = META.version
    ...
    _cdx_sbom.components.add(
        new CDX.Models.Component(
            ...
            {
                version: `${msedit_version}`,
                cpe: `cpe:2.3:*:*:msedit:${msedit_version}:*:*:*:*:*:*:*`,
                purl: PackageURL.fromString(`pkg:generic/msedit@${msedit_version}`)
            }
        )
    )
    ...
    logger.success("SBOM generation for MSEdit completed successfully.")
}
